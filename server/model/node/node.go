...
const StatusServicing = "servicing"
...
type Node struct {
	...
	BootTime             string `json:"bootTime" gorm:"column:boot_time;type:string;size:128;comment:运行时间"`
	MemTotalBytes        string `json:"memTotalBytes" gorm:"column:mem_total_bytes;type:string;size:128;comment:总内存"`
	CpuCount             string `json:"cpuCount" gorm:"column:cpu_count;type:string;size:32;comment:总核数"`
	Load5                string `json:"load5" gorm:"column:load5;type:string;size:32;comment:5分钟负载"`
	CPUAvailable         string `json:"cpuAvailable" gorm:"column:cpu_available;type:string;size:32;comment:cpu利用率"`
	MemAvailable         string `json:"memAvailable" gorm:"column:mem_available;type:string;size:32;comment:内存利用率"`
	FileSystemAvailable  string `json:"fileSystemAvailable" gorm:"column:file_system_available;type:string;size:32;comment:分区利用率"`
	MaxDiskReadBytes     string `json:"maxDiskReadBytes" gorm:"column:max_disk_read_bytes;type:string;size:32;comment:最大读取"`
	MaxDiskWrittenBytes  string `json:"maxDiskWrittenBytes" gorm:"column:max_disk_read_bytes;type:string;size:32;comment:最大写入"`
